...
function SessionManager() {
  ...
  const generateSoapNoteMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await apiRequest(
        "POST",
        `/api/sessions/${id}/generate-soap-note`
      );
      return await response.json();
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        queryKey: ["/api/sessions", data.session.id],
      });
  ...
  const saveToClinicalNotesMutation = useMutation({
    mutationFn: async (sessionId: number) => {
      const session = await queryClient.getQueryData<PatientSession>([
        "/api/sessions",
        sessionId,
      ]);
            ...
            <TranscriptView
              transcript={
                session.transcriptUrl
                  ? "Transcript loaded from server. Click 'Generate SOAP Note' to proceed."
                  : ""
              }
              isLoading={false}
          ...
          {sessions.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-muted-foreground max-w-md mb-6">
                Create a new patient session to start recording and generating
                SOAP notes.
              </p>
              <Button onClick={() => navigate("/notes/new")}>
                <Plus className="h-4 w-4 mr-2" />
