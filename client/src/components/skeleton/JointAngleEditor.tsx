...
function EditorRow({ spec, value, pairValue, onChange, onJumpToJoint, bilateralLink }: RowProps) {
  ...
  return (
    <div
      className={`group p-2 rounded-md border transition-colors ${pulse ? 'border-primary/60 bg-primary/5' : 'border-transparent hover:bg-muted/40'}`}
      onWheel={handleWheel}
    ...
    return (
      <div className="space-y-3">
        {Array.from(sections.entries()).map(([section, rows]) => (
          <div key={section}>
            <div className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground px-2 mb-1">{section}</div>
