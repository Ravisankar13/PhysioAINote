        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="findings" className="p-6">
            {!hasDiagnosis ? (
              ...
              <div className="space-y-6">
                {patient.objectiveFindings && patient.objectiveFindings.map((finding: any, idx: number) => (
                  <div key={idx} className="border rounded-lg p-4">
                    <h4 className="text-md font-semibold mb-3 bg-primary/10 p-2 rounded">
                      {finding.category}
                    </h4>
                    {finding.observations && finding.observations.map((observation: any, obsIdx: number) => (
                      <div key={obsIdx} className="mb-4">
                        <h5 className="text-sm font-medium mb-1">{observation.title}</h5>
                        <p className="text-sm mb-2">{observation.details}</p>
                        
                        {observation.measurements && (
                          <div className="grid grid-cols-2 gap-2 text-sm ml-3 mt-2">
                            {Object.entries(observation.measurements).map(([key, value]: [string, any], i: number) => (
                        ...
                        {observation.notes && (
                          <p className="text-sm italic mt-2">{observation.notes}</p>
                        )}
                      </div>
                    ))}
                  </div>
                    ...
                    {patient.treatmentOptions && patient.treatmentOptions.map((treatmentCategory: any, idx: number) => (
                      <div key={idx} className="border rounded-lg p-4">
                        ...
                        {treatmentCategory.recommendations && treatmentCategory.recommendations.length > 0 && (
                          <div className="space-y-3">
                            {treatmentCategory.recommendations.map((rec: any, eduIdx: number) => (
                              <div key={eduIdx} className="border border-gray-200 rounded-md p-3">
                                <div className="flex justify-between items-start mb-2">
                                  <h5 className="text-md font-medium">{rec.topic}</h5>
                                  <Badge variant={
                                    rec.importance === "high" ? "default" : 
                                    rec.importance === "medium" ? "secondary" : 
                                    "outline"
                                  }>
