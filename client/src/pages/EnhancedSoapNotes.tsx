  ...
  const analyzePainLocations = async (transcript: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setVirtualPatientParams(params);
      }
    } catch (error) {
  ...
  const startRecording = async () => {
    try {
      ...
      if (userData?.id) {
        ...
        // Initial analysis if we have existing transcript
        if (realTimeTranscript && realTimeTranscript.length > 20) {
          analyzePainLocations(realTimeTranscript);
        }
        ...
        const pollInterval = setInterval(() => {
          ...
          // Get current transcript from real-time or progressive transcript
          const currentTranscript = progressiveTranscript || realTimeTranscript || "";
          
          if (currentTranscript && currentTranscript.length > 20) {
