    ...
    try {
      ...
      if (fileSizeMB > MAX_CHUNK_SIZE_MB) {
        ...
        if (data.soapNote) {
          ...
          if (temporaryNoteId && !isContinuousMode) {
            const updateData = {
              soapSections: data.soapNote,
              transcript: transcript,
              sessionDuration: recordingTime * 1000, // Convert to milliseconds
              patientName: data.soapNote.patientName || 'Patient'
            };
            updateTemporaryNoteMutation.mutate({ id: temporaryNoteId, data: updateData });
          }
    ...
    try {
      ...
      if (!isContinuousMode) {
        ...
        const noteData = {
          ...
          patientName: 'Recording in progress...',
          soapSections: {
            subjective: '',
            objective: '',
            assessment: '',
      ...
      chunkIntervalRef.current = setInterval(() => {
        ...
        if (recentChunks.length > 0 && isRecordingRef.current) {
          ...
          const intermediateBlob = new Blob(chunks.slice(), { type: mimeType });
          console.log(`Processing intermediate blob of size: ${intermediateBlob.size} bytes with MIME type: ${mimeType}`);
      ...
      recorder.onstop = () => {
        ...
        if (blob.size > 0) {
          console.log(`Final recording size: ${(blob.size / 1024 / 1024).toFixed(2)}MB with MIME type: ${mimeType}`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white p-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Temporary Notes Section (24-hour storage) */}
            ...
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-amber-900">Temporary Notes (24-hour storage)</span>
                </div>
                <Badge variant="outline" className="bg-amber-100 text-amber-700 border-amber-300">
                  {temporaryNotesData.length} {temporaryNotesData.length === 1 ? 'note' : 'notes'}
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {temporaryNotesData.map((note: any) => {
                  ...
                  return (
                    <div 
                      key={note.id} 
                      className="p-4 bg-white rounded-lg border border-amber-200 hover:shadow-md transition-shadow cursor-pointer"
                      onClick={() => {
                        // Load the temporary note into the current session
                        if (note.soapSections) {
                          setSoapSections(note.soapSections);
                          setRealTimeTranscript(note.transcript || '');
                          setTemporaryNoteId(note.id);
                          toast({
                            title: "Temporary Note Loaded",
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className={`flex items-center gap-1 text-sm ${isExpiringSoon ? 'text-red-600' : 'text-amber-600'}`}>
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        ...
                        {note.sessionDuration && (
                          <div className="flex items-center gap-2">
                            ...
                            <span>Duration: {formatTime(Math.floor(note.sessionDuration / 1000))}</span>
                          </div>
                        )}
                      </div>
                      
                      {note.soapSections?.subjective && (
                        <div className="mt-2 text-xs text-gray-500 line-clamp-2">
                          {note.soapSections.subjective}
                        </div>
                      )}
                      
                      {note.id === temporaryNoteId && (
              ...
              <div className="mt-4 p-3 bg-amber-100 rounded-lg">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm text-amber-800">
                    <p className="font-medium">Temporary Storage Notice</p>
                    <p className="text-xs mt-1">
                      These notes are automatically deleted after 24 hours. Save important notes permanently before they expire.
                    </p>
                  </div>
                </div>
              </div>
