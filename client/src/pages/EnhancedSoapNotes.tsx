...
Generated by PhysioGPT Enhanced SOAP Notes
  ...
  const processCompleteAudioRecording = async (audioBlob: Blob) => {
    try {
      ...
      const MAX_CHUNK_SIZE_MB = 20; // OpenAI Whisper limit is 25MB, use 20MB for safety
      ...
      if (fileSizeMB > MAX_CHUNK_SIZE_MB) {
        ...
        setRealTimeTranscript(fileSizeMB > 5 
          ? "Processing recording... This may take 30-60 seconds."
  ...
  const startRecording = async () => {
    try {
      ...
      let chunkCount = 0;
      const CHUNK_INTERVAL = 30; // Process chunks every 30 seconds for progressive transcription
      ...
      recorder.ondataavailable = (e) => {
        if (e.data.size > 0) {
          ...
          // Process intermediate chunks for real-time feedback (every 30 seconds)
          if (chunkCount >= CHUNK_INTERVAL) {
            const intermediateBlob = new Blob([...chunks.slice(-CHUNK_INTERVAL)], { type: "audio/webm" });
            processIntermediateAudio(intermediateBlob);
            chunkCount = 0; // Reset counter but keep all chunks for final processing
          }
        }
      };
      ...
      toast({
        ...
        description: `${isContinuousMode ? "Continuous" : "Audio"} recording in progress. Can record up to 45-minute sessions with optimized chunking.`,
      });
      ...
      if (userData?.id) {
        ...
        connectWebSocket(newSessionId, userData.id);
        
        // Initialize WebSocket for real-time virtual patient
        const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
        const wsUrl = `${protocol}//${window.location.host}/ws/soap-ai?sessionId=${newSessionId}&userId=${userData.id}`;
        const ws = new WebSocket(wsUrl);
        
        ws.onopen = () => {
          console.log('WebSocket connected for real-time virtual patient');
          setIsWebSocketConnected(true);
        };
