...
Generated by PhysioGPT Enhanced SOAP Notes
  ...
  const startRecording = async () => {
    ...
    if (!sessionStartTime) {
      ...
      // Recording time will continue from current totalSessionTime
    } else {
    ...
    // Start timer - in continuous mode, track total session time
    ...
    recognitionRef.current.onresult = (event: any) => {
      ...
      // Process transcript chunks for continuous recording
      if (isContinuousMode && continuousSession && finalTranscript.length > lastProcessedLength) {
        const newChunk = finalTranscript.slice(lastProcessedLength);
        if (newChunk.trim().length > 0) {
          ...
          lastProcessedLength = finalTranscript.length;
        }
      }
    };
    ...
    recognitionRef.current.onend = () => {
      if (isRecording) {
        // Restart recognition if still recording (especially important for continuous mode)
        setTimeout(() => {
