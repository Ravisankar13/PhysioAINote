...
export default function EnhancedSoapNotesPage() {
  ...
  const processTranscriptMutation = useMutation({
    mutationFn: async ({ sessionId, transcript, duration }: { sessionId: string, transcript: string, duration: number }) => {
      const response = await fetch('/api/continuous-recording/process-transcript', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sessionId, transcript, duration }),
      });
    ...
    onSuccess: (detectionResult) => {
      if (detectionResult.isNewPatient) {
        const newPatient = {
          patientNumber: detectionResult.suggestedPatientNumber,
          confidence: detectionResult.confidence,
          reason: detectionResult.reason,
          timestamp: new Date().toISOString(),
        };
        setDetectedPatients(prev => [...prev, newPatient]);
        setCurrentPatientNumber(detectionResult.suggestedPatientNumber);
        
        toast({
          title: `New Patient Detected!`,
          description: `Patient ${detectionResult.suggestedPatientNumber} (${detectionResult.confidence}% confidence)`,
        });
      }
      
      // Track detection history for debugging
      setPatientDetectionHistory(prev => [...prev.slice(-20), detectionResult]); // Keep last 20 results
    },
  });
...
Generated by PhysioGPT Enhanced SOAP Notes
  ...
  const processCompleteAudioRecording = async (audioBlob: Blob) => {
    try {
      ...
      formData.append('sessionId', `enhanced-soap-${Date.now()}`);

      // Send to backend for transcription and SOAP generation
      const response = await fetch('/api/soap-notes/transcribe-and-generate', {
        method: 'POST',
        body: formData,
      });
      ...
      // Update transcript and SOAP sections
      setRealTimeTranscript(data.transcript || "Transcription completed");
      
      if (data.soapSections) {
        setSoapSections({
          subjective: data.soapSections.subjective || "",
          objective: data.soapSections.objective || "Physical examination findings to be documented...",
          assessment: data.soapSections.assessment || "Clinical assessment based on findings...",
          plan: data.soapSections.plan || "Treatment plan to be developed..."
        });
      }

      toast({
        title: "Audio Processed",
        description: "AI has transcribed your recording and generated SOAP sections.",
      });

    } catch (error) {
      console.error('Error processing audio:', error);
      setRealTimeTranscript("Audio processing failed. Please try recording again.");
  ...
  const startRecording = async () => {
    try {
      ...
      if (!sessionStartTime) {
        ...
        // In continuous mode, don't reset timing - resume from where we left off
      } else {
        // In single patient mode, reset timing
        setRecordingTime(0);
        setSessionStartTime(Date.now());
      }
      
      setRealTimeTranscript("Recording audio... Full transcription will be generated when recording stops (supports 30+ minute sessions).");
      
      // Start timer
      intervalRef.current = setInterval(() => {
        if (isContinuousMode && sessionStartTime) {
          // Calculate actual elapsed time since session start
          const actualElapsed = Math.floor((Date.now() - sessionStartTime) / 1000);
          setTotalSessionTime(actualElapsed);
          setRecordingTime(actualElapsed);
        } else {
  ...
  const stopRecording = () => {
    if (isContinuousMode) {
      ...
      // Pause timer but preserve session time
      if (intervalRef.current) {
        clearInterval(intervalRef.current);
      }
