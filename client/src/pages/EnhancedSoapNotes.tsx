...
Generated by PhysioGPT Enhanced SOAP Notes
  ...
  const processIntermediateAudio = async (audioBlob: Blob) => {
    try {
      ...
      formData.append('isCompleteAudio', 'true'); // Flag to indicate this is complete audio so server extracts new portion
  ...
  const startRecording = async () => {
    try {
      ...
      chunkIntervalRef.current = setInterval(() => {
        console.log(`30-second interval triggered. Chunks available: ${chunks.length}, Recording: ${isRecordingRef.current}`);
        if (chunks.length > 0 && isRecordingRef.current) {
          // Create blob from ALL chunks to ensure valid webm file with headers
          // The server will handle extracting only the new portion
          const intermediateBlob = new Blob(chunks, { type: mimeType });
          console.log(`Processing complete audio blob of size: ${intermediateBlob.size} bytes with MIME type: ${mimeType}`);
          
          // Process for real-time SOAP generation
          processIntermediateAudio(intermediateBlob);
          
          // Don't clear chunks - we need to keep all for valid webm format
          // Clear only the recent chunks tracker
          recentChunks = [];
        }
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-4 flex justify-end">
            <div className="flex items-center gap-2 bg-white p-2 rounded-lg shadow-sm">
              ...
              <Button
                variant={useStreamingMode ? "outline" : "secondary"}
                size="sm"
              ...
              <Button
                variant={useStreamingMode ? "secondary" : "outline"}
                size="sm"
