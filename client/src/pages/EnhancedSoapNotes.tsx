...
Generated by PhysioGPT Enhanced SOAP Notes
  ...
  const createVirtualPatientFromSOAP = async (includeMotionData: boolean = false) => {
    ...
    try {
      const requestData = {
        ...
        sessionDuration: recordingTime,
        includeMotionData,
        motionCaptureData: includeMotionData ? capturedMotionData : null,
      };
  ...
  const handleMotionCaptureComplete = (motionData: MotionCaptureData) => {
    setCapturedMotionData(motionData);
    setShowMotionCaptureIntegration(false);
    
    toast({
      title: "Motion Data Captured",
      description: `Captured ${motionData.analysis.totalFrames} frames of movement data. Ready to create digital twin.`,
    });
  };
                  ...
                  {capturedMotionData && (
                    <div className="mt-3 p-3 bg-green-50 rounded-lg border border-green-200">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium text-green-800">Motion Data Available</span>
                      </div>
                      <p className="text-sm text-green-700">
                        {capturedMotionData.analysis.totalFrames} motion frames captured 
                        ({(capturedMotionData.analysis.duration / 1000).toFixed(1)}s recording)
                      </p>
                      <div className="flex gap-2 mt-2">
                        <Button 
                          size="sm" 
                          variant="default"
                          onClick={() => createVirtualPatientFromSOAP(true)}
                          className="text-xs"
                        >
                          Create Digital Twin Now
                        </Button>
                        <Button 
                          size="sm" 
                          variant="outline"
                          onClick={() => setCapturedMotionData(null)}
                          className="text-xs"
                        >
                          Clear Motion Data
                        </Button>
