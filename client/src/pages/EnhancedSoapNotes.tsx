...
export default function EnhancedSoapNotesPage() {
  ...
  useEffect(() => {
    if (realTimeTranscript && realTimeTranscript.length > 100) {
      ...
      // Send transcript update to WebSocket for virtual patient
...
Generated by PhysioGPT Enhanced SOAP Notes
  ...
  const startRecording = async () => {
    try {
      ...
      if (userData?.id) {
        const sessionId = continuousSession?.sessionId || "demo-session";
        connectWebSocket(sessionId, userData.id);
        
        // Initialize WebSocket for real-time virtual patient
        const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
        const wsUrl = `${protocol}//${window.location.host}/ws/soap-ai?sessionId=${sessionId}&userId=${userData.id}`;
