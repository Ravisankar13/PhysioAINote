...
Generated by PhysioGPT Enhanced SOAP Notes
  ...
  const startRecording = async () => {
    try {
      ...
      // Start simple counter that increments every second
      let seconds = 0;
      console.log('Starting timer...');
      
      // Use window.setInterval to ensure global scope
      intervalRef.current = window.setInterval(() => {
        seconds++;
        console.log('Timer tick:', seconds);
        setRecordingTime(current => {
          console.log('Setting recording time from', current, 'to', seconds);
          return seconds;
        });
        if (isContinuousMode) {
      ...
      setTimeout(() => {
        console.log('Test update after 100ms');
        setRecordingTime(1);
      }, 100);
