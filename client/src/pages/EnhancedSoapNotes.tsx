              ...
              <Card>
                <CardHeader className="pb-2 pt-3 px-3">
                  <CardTitle className="flex items-center justify-between text-sm">
                    ...
                    {lastEvidenceUpdate && (
                      <span className="text-xs text-gray-500">{lastEvidenceUpdate}</span>
                    )}
                  </CardTitle>
                </CardHeader>
                <CardContent className="pt-0 px-3 pb-3">
                  {isLoadingEvidence ? (
                    <div className="flex items-center justify-center py-4">
                      ...
                      <span className="text-xs text-gray-600">Finding research...</span>
                    </div>
                  ) : evidenceArticles.length > 0 ? (
                    <div className="space-y-2 max-h-[250px] overflow-y-auto">
                      {evidenceArticles.map((article) => (
                        <div key={article.id} className="p-2 border rounded-lg hover:bg-gray-50 transition-colors">
                          <div className="flex items-start justify-between mb-1">
                            <h4 className="font-medium text-xs text-gray-900 flex-1 pr-2">{article.title}</h4>
                            <Badge variant="outline" className={`text-xs shrink-0 ${article.evidenceLevel === 'high' ? 'border-green-500 text-green-700' : article.evidenceLevel === 'moderate' ? 'border-yellow-500 text-yellow-700' : 'border-gray-500 text-gray-700'}`}>{article.evidenceLevel}</Badge>
                          </div>
                          <p className="text-xs text-gray-600 mb-1">{article.authors} ({article.year})</p>
                          <p className="text-xs text-gray-700 line-clamp-2">{article.summary}</p>
                          <Collapsible>
                            <CollapsibleTrigger asChild>
                              <Button variant="ghost" size="sm" className="text-xs text-purple-600 hover:text-purple-700 p-0 h-auto mt-1">
                            ...
                            <CollapsibleContent className="mt-1 space-y-1">
                              {article.keyFindings.length > 0 && (
                                <div className="bg-blue-50 p-1.5 rounded">
                                  <p className="text-xs font-medium text-blue-800 mb-0.5">Key Findings:</p>
                                  <ul className="text-xs text-blue-700 space-y-0.5">
                                    {article.keyFindings.map((finding, idx) => (<li key={idx} className="flex items-start"><span className="mr-1">•</span><span>{finding}</span></li>))}
                                  </ul>
                                </div>
                              )}
                              {article.clinicalApplication && (
                                <div className="bg-green-50 p-1.5 rounded">
                                  <p className="text-xs font-medium text-green-800 mb-0.5">Clinical Application:</p>
                                  <p className="text-xs text-green-700">{article.clinicalApplication}</p>
                                </div>
                              )}
                              <div className="flex items-center justify-between pt-1">
                                <Badge variant="outline" className="text-xs">{article.sourceType}</Badge>
                      ...
                      <Button variant="outline" size="sm" className="w-full mt-1 text-xs" onClick={() => generateEvidenceArticles()} disabled={isLoadingEvidence}>
                        <RefreshCw className={`w-3 h-3 mr-1.5 ${isLoadingEvidence ? 'animate-spin' : ''}`} />Refresh
                      </Button>
                    </div>
                  ) : (
                    <div className="text-center py-4">
                      <BookOpen className="w-8 h-8 text-gray-300 mx-auto mb-2" />
                      <p className="text-xs text-gray-600">No research articles yet</p>
                      <p className="text-xs text-gray-500">Start recording to see evidence</p>
                    </div>
                  )}
                </CardContent>
              </Card>
              ...
              <Card className="py-2">
                <CardContent className="px-3 py-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isRecording && (
                      <span className="text-xs font-bold text-red-600 shrink-0">{formatTime(recordingTime)}</span>
                    )}
                    <Button
                    ...
                    >
                      {isRecording ? (
                        isContinuousMode ? "Pause" : "Stop"
                      ) : (
                        isContinuousMode && continuousSession ? "Continue" : "Start Recording"
                      )}
                    </Button>
                    {isRecording && <Badge variant="destructive" className="animate-pulse text-xs py-0 h-5 shrink-0">REC</Badge>}
                  </div>
                  {realTimeTranscript && (
                    <div className="mt-1.5 p-1.5 bg-gray-50 rounded text-left">
                      <div className="max-h-10 overflow-y-auto text-xs text-gray-700 leading-snug">
                        {realTimeTranscript}
                      </div>
                    </div>
                  )}
              ...
              <Card>
                ...
                <CardContent className="pt-0 px-3 pb-3">
                  <div className="grid grid-cols-2 gap-1">
                    <Button variant="outline" size="sm" className="justify-start text-xs h-7 hover:bg-blue-50" onClick={generateDoctorReport}>
                      <FileText className="w-3 h-3 mr-1 shrink-0" />
                      Doctor Report
                    </Button>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-7 hover:bg-green-50" onClick={generateAHTR}>
                      <FileCheck className="w-3 h-3 mr-1 shrink-0" />
                      AHTR PDF
                    </Button>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-7 hover:bg-purple-50" onClick={generateImagingReferral}>
                      <Shield className="w-3 h-3 mr-1 shrink-0" />
                      Imaging Referral
                    </Button>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-7 hover:bg-indigo-50" onClick={generateDischargeSummary}>
                      <FileCheck className="w-3 h-3 mr-1 shrink-0" />
                      Discharge Summary
                    </Button>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-7 hover:bg-emerald-50" onClick={generateProgressReport}>
                      <BarChart3 className="w-3 h-3 mr-1 shrink-0" />
                      Progress Report
                    </Button>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-7 hover:bg-cyan-50" onClick={generateSpecialistReferral}>
                      <Users className="w-3 h-3 mr-1 shrink-0" />
                      Specialist Referral
                    </Button>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-7 hover:bg-lime-50" onClick={generateReturnToWork}>
                      <Briefcase className="w-3 h-3 mr-1 shrink-0" />
                      Return to Work
                    </Button>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-7 hover:bg-amber-50" onClick={generateTimeOffWork}>
                      <Clock className="w-3 h-3 mr-1 shrink-0" />
                      Time Off Work
                    </Button>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-7 hover:bg-yellow-50" onClick={generateInsuranceDocumentation}>
                      <DollarSign className="w-3 h-3 mr-1 shrink-0" />
                      Insurance Docs
                    </Button>
                    <Button variant="outline" size="sm" className="justify-start text-xs h-7 hover:bg-rose-50" onClick={() => setShowForumShareDialog(true)} disabled={!soapSections.subjective && !soapSections.objective && !soapSections.assessment && !soapSections.plan}>
                      <Users className="w-3 h-3 mr-1 shrink-0" />
                      Share to Forum
                    </Button>
                  </div>
