    ...
    try {
      ...
      if (response.ok) {
        ...
        if (chunkTranscript && chunkTranscript.length > 10) {
          ...
          if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
            console.log('Sending realtime_update via WebSocket with transcript length:', updatedTranscript.length);
            wsRef.current.send(JSON.stringify({
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (data.type === 'ai_suggestions') {
              ...
              console.log('Virtual patient update received in main component');
            }
          } catch (error) {
