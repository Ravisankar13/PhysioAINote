...
Generated by PhysioGPT Enhanced SOAP Notes
  ...
  const processIntermediateAudio = async (audioBlob: Blob) => {
    try {
      ...
      formData.append('isCompleteAudio', 'false'); // Flag to indicate this is an incremental chunk, not complete audio
    ...
    try {
      ...
      chunkIntervalRef.current = setInterval(() => {
        ...
        if (recentChunks.length > 0 && isRecordingRef.current) {
          // Create blob from ONLY the recent chunks (last 30 seconds)
          // This prevents sending increasingly large files as recording continues
          const intermediateBlob = new Blob(recentChunks, { type: mimeType });
          console.log(`Processing intermediate blob of size: ${intermediateBlob.size} bytes (last 30 seconds) with MIME type: ${mimeType}`);
