...
export default function EnhancedSoapNotesPage() {
  ...
  // State for WebSocket connection to trigger re-renders
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (chunkTranscript && chunkTranscript.length > 10) {
          ...
          if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
            console.log('[WebSocket] Sending realtime_update with transcript length:', updatedTranscript.length);
            try {
              wsRef.current.send(JSON.stringify({
              ...
              console.error('[WebSocket] Error sending realtime_update:', error);
            }
          } else {
            console.log('[WebSocket] Not ready - state:', wsRef.current?.readyState, '(0=CONNECTING, 1=OPEN, 2=CLOSING, 3=CLOSED)');
          }
    ...
    try {
      ...
      if (userData?.id) {
        ...
        ws.onopen = () => {
          ...
          // Wait a bit before sending first message to ensure connection is stable
          setTimeout(() => {
            if (ws.readyState === WebSocket.OPEN) {
              console.log('[WebSocket] Sending connection_init message');
              try {
        ...
        ws.onclose = (event) => {
          ...
          // Don't reconnect if we intentionally closed or recording stopped
          ...
          if (reconnectAttempts < maxReconnectAttempts) {
            reconnectAttempts++;
            const reconnectDelay = Math.min(1000 * Math.pow(2, reconnectAttempts - 1), 5000);
            
            setTimeout(() => {
              console.log(`Attempting to reconnect WebSocket... (attempt ${reconnectAttempts}/${maxReconnectAttempts})`);
              
              // Only reconnect if we're still recording and WebSocket is not already connected
              if (!isRecordingRef.current) {
                console.log('Recording stopped, skipping reconnection');
                return;
              }
              
              if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
                console.log('WebSocket already open, skipping reconnection');
                return;
              }
              ...
              try {
                ...
                reconnectWs.onopen = () => {
                  ...
                  // Send init message after reconnection
                  setTimeout(() => {
                    if (reconnectWs.readyState === WebSocket.OPEN) {
                      try {
                        ...
                        console.error('[WebSocket] Error sending reconnect init:', error);
                      }
                    }
                  }, 100);
                };
                ...
                setWebSocketConnection(reconnectWs);
              } catch (error) {
                console.error('[WebSocket] Failed to create reconnection:', error);
              }
            }, reconnectDelay);
          } else {
            ...
            toast({
              ...
              description: "Real-time features may be limited. Please refresh if issues persist.",
              variant: "destructive"
            });
          }
        };
        ...
        ws.onmessage = (event) => {
          try {
            ...
            } else if (data.type === 'ai_suggestions') {
              if (data.suggestions && data.suggestions.length > 0) {
                setAiSuggestions(data.suggestions);
              }
            } else if (data.type === 'virtual_patient_update') {
              // Virtual patient updates are handled by the RealtimeVirtualPatient component
              console.log('[WebSocket] Virtual patient update received, parameters:', data.parameters);
            }
          } catch (error) {
