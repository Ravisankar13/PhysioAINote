    ...
    try {
      ...
      if (userData?.id) {
        ...
        ws.onclose = (event) => {
          ...
          // Simple reconnection without creating duplicate connections
          if (reconnectAttempts < maxReconnectAttempts) {
            reconnectAttempts++;
            console.log(`Will attempt reconnection ${reconnectAttempts}/${maxReconnectAttempts} in 2 seconds...`);
            
            // Clear existing WebSocket reference to prevent duplicates
            wsRef.current = null;
            setWebSocketConnection(null);
            ...
            setTimeout(() => {
              ...
              if (!isRecordingRef.current) {
                console.log('Recording stopped, canceling reconnection');
                return;
              }
              
              // Check if we haven't already reconnected
              if (wsRef.current) {
                console.log('WebSocket already exists, skipping reconnection');
                return;
              }
              
              console.log(`Reconnecting WebSocket (attempt ${reconnectAttempts}/${maxReconnectAttempts})...`);
              
              // Create new WebSocket connection only
            ...
            toast({
              ...
              description: "Unable to maintain real-time connection. Please stop and restart recording.",
              variant: "destructive"
            });
          }
        };
