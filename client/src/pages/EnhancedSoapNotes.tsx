...
Generated by PhysioGPT Enhanced SOAP Notes
  ...
  const processCompleteAudioRecording = async (audioBlob: Blob) => {
    try {
      ...
      setRealTimeTranscript("Processing audio recording...");
      
      // Wait 2 seconds to simulate processing
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      const testTranscript = `Recording completed successfully. Duration: ${formatTime(recordingTime)}, Size: ${(audioBlob.size / 1024).toFixed(2)}KB`;
      
      setRealTimeTranscript(testTranscript);
      
      // Generate test SOAP sections
      setSoapSections({
        subjective: "Test recording demonstrates functional audio capture.",
        objective: `Audio recorded for ${formatTime(recordingTime)} with ${(audioBlob.size / 1024).toFixed(2)}KB of data.`,
        assessment: "Recording system working correctly.",
        plan: "Ready for full transcription implementation."
      });

      toast({
        title: "Recording Processed",
        description: "Audio capture completed successfully.",
      });
  ...
  const startRecording = async () => {
    try {
      ...
      // Initialize session timing - set to 1 to start
      const startTime = Date.now();
      setSessionStartTime(startTime);
      setRecordingTime(1); // Start at 1 to avoid 0 display
      setTotalSessionTime(0);
      ...
      // Force immediate update and then start interval
      let currentTime = 1;
      setRecordingTime(currentTime);
      
      intervalRef.current = setInterval(() => {
        currentTime += 1;
        console.log('Timer update:', currentTime); // Debug log
        setRecordingTime(currentTime);
        if (isContinuousMode) {
          setTotalSessionTime(currentTime);
        }
      }, 1000);
