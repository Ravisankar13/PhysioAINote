    ...
    try {
      ...
      if (response.ok) {
        ...
        if (chunkTranscript && chunkTranscript.length > 10) {
          ...
          // Virtual patient updates will be handled by polling
  ...
  const startRecording = async () => {
    try {
      ...
      if (userData?.id) {
        ...
        // Test with a sample pain description immediately
        setTimeout(() => {
          console.log('[PainAnalysis] Testing with sample pain description');
          analyzePainLocations("The patient is experiencing severe pain in their right shoulder when lifting their arm. They also mention lower back pain that radiates down to their left knee.");
        }, 2000);
        ...
        const pollInterval = setInterval(() => {
          ...
          if (currentTranscript && currentTranscript.length > 20 && !currentTranscript.includes("🎙️")) {
            console.log('[PainAnalysis] Analyzing transcript:', currentTranscript.substring(0, 100) + '...');
            analyzePainLocations(currentTranscript);
          } else {
            console.log('[PainAnalysis] Waiting for transcript data...', currentTranscript.length);
          }
