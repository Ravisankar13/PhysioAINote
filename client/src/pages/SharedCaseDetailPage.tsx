...
export default function SharedCaseDetailPage() {
  ...
  >({
    ...
    queryFn: () =>
      fetch(`/api/shared-cases/${caseId}/discussions`).then((res) =>
        res.json()
      ),
    refetchOnWindowFocus: false,
  });
  ...
  const upvoteMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest(
        "POST",
        `/api/shared-cases/${caseId}/upvote`
      );
      return res.json();
    },
    onSuccess: () => {
  ...
  const removeUpvoteMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description:
          error.message || "Failed to remove upvote. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const addCommentMutation = useMutation({
    mutationFn: async (data: { content: string; parentId?: number }) => {
      const res = await apiRequest(
        "POST",
        `/api/shared-cases/${caseId}/discussions`,
        data
      );
      return res.json();
    },
  ...
  const upvoteDiscussionMutation = useMutation({
    mutationFn: async (discussionId: number) => {
      const res = await apiRequest(
        "POST",
        `/api/case-discussions/${discussionId}/upvote`
      );
      return res.json();
    },
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast({
        title: "Error",
        description:
          error.message || "Failed to upvote comment. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const bodyPartLabel = caseData
    ? BODY_PARTS.find((part) => part.value === caseData.bodyPart)?.label ||
      caseData.bodyPart
    : "";
  const expertiseLabel = caseData
    ? EXPERTISE_LEVELS.find((level) => level.value === caseData.expertiseLevel)
        ?.label || caseData.expertiseLevel
    : "";
  const complexityLabel = caseData
    ? COMPLEXITY_LEVELS.find(
        (level) => level.value === caseData.complexityLevel
      )?.label || caseData.complexityLevel
    : "";

  // Format dates
  const createdDate = caseData
  ...
  if (!caseData) {
    return (
      <div className="container py-12 text-center">
        ...
        <Button onClick={() => navigate("/shared-cases")}>Back to Cases</Button>
      </div>
    );
  }
              ...
              <div className="space-y-6">
                {discussions.map((discussion) => (
                  <CommentCard
                    ...
                    onUpvote={() =>
                      upvoteDiscussionMutation.mutate(discussion.id)
                    }
                    onRemoveUpvote={() =>
                      removeUpvoteDiscussionMutation.mutate(discussion.id)
                    }
                    currentUser={user}
                  />
                ))}
              ...
              <div className="flex items-center space-x-3">
                  ...
                  {caseData.user?.profileImage ? (
                    ...
                    <AvatarFallback>
                      {caseData.user?.username
                        ? caseData.user.username.substring(0, 2).toUpperCase()
                        : "PT"}
                    </AvatarFallback>
                  )}
                </Avatar>
                <div>
                  <p className="font-medium">
                    {caseData.user?.username || "Anonymous"}
                  </p>
...
function CommentCard({
  ...
  const { data: replies = [] } = useQuery<CaseDiscussion[]>({
    ...
    queryFn: () =>
      fetch(`/api/case-discussions/${discussion.id}/replies`).then((res) =>
        res.json()
      ),
    enabled: showReplies,
    refetchOnWindowFocus: false,
    ...
    <div className="border rounded-md p-4">
      <div className="flex gap-4">
          ...
          {discussion.user?.profileImage ? (
            ...
            <AvatarFallback>
              {discussion.user?.username
                ? discussion.user.username.substring(0, 2).toUpperCase()
                : "PT"}
            </AvatarFallback>
          )}
        </Avatar>
        ...
        <div className="flex-1">
          ...
          <div className="mt-2 whitespace-pre-line">{discussion.content}</div>
                          ...
                          onClick={() =>
                            currentUser
                              ...
                              : toast({
                                  title: "Authentication required",
                                  description:
                                    "Please sign in to upvote replies",
                                })
                          }
                          className="flex items-center"
                        >
