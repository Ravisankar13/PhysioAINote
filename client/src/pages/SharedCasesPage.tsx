...
import { Loader2, ThumbsUp, Eye, MessageSquare, Tag, Bookmark, Filter, Search } from 'lucide-react';
...
export default function SharedCasesPage() {
  ...
  const { 
    ...
    queryFn: () => {
      ...
      return fetch(`/api/shared-cases?${params.toString()}`)
        .then(res => res.json());
    },
  ...
  const { 
    ...
    queryFn: () => {
      return fetch('/api/my-shared-cases')
        .then(res => res.json());
    },
    enabled: !!user,
    refetchOnWindowFocus: false
    ...
    <div className="container py-8">
      ...
      <Tabs defaultValue="all">
          ...
          <div className="space-y-4">
            ...
            {filter && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 border rounded-md bg-muted/30">
                ...
                <div>
                  <label className="text-sm font-medium mb-1 block">Expertise Level</label>
            ...
            {(isLoadingCases || isFetchingCases) ? (
              ...
              <div className="text-center py-12 border rounded-md">
                ...
                {(bodyPart || expertiseLevel || complexityLevel || searchTerm) && (
                  <Button
                    variant="outline"
                    onClick={resetFilters}
                    className="mt-4"
                    ...
                    <PaginationContent>
                      ...
                      <PaginationItem>
                        <PaginationNext 
                          ...
                          className={page >= Math.ceil(sharedCasesData.total / pageSize) ? "pointer-events-none opacity-50" : "cursor-pointer"}
                        />
                      </PaginationItem>
                    </PaginationContent>
                  </Pagination>
        ...
        <TabsContent value="my">
          {!user ? (
            <div className="text-center py-12 border rounded-md">
              <p className="text-lg text-muted-foreground">Please log in to view your cases.</p>
              <Button
                variant="outline"
            ...
            <div className="text-center py-12 border rounded-md">
              <p className="text-lg text-muted-foreground">You haven't shared any cases yet.</p>
              <Button
...
function CaseCard({ caseItem, isMine = false }: { caseItem: SharedCase; isMine?: boolean }) {
  // Find the display names for the enum values
  const bodyPartLabel = BODY_PARTS.find(part => part.value === caseItem.bodyPart)?.label || caseItem.bodyPart;
  const expertiseLabel = EXPERTISE_LEVELS.find(level => level.value === caseItem.expertiseLevel)?.label || caseItem.expertiseLevel;
  const complexityLabel = COMPLEXITY_LEVELS.find(level => level.value === caseItem.complexityLevel)?.label || caseItem.complexityLevel;
    ...
    <Card className="overflow-hidden">
        ...
        <div className="flex justify-between items-start">
          <div className="space-y-1">
            <CardTitle className="text-xl">
              <Link href={`/shared-cases/${caseItem.id}`} className="hover:underline">
                {caseItem.title}
              </Link>
            </CardTitle>
            <CardDescription>
          ...
          {isMine && (
            <Badge variant="secondary">My Case</Badge>
          )}
        </div>
      </CardHeader>
      ...
      <CardFooter className="flex justify-between pt-0">
        ...
        <Button variant="ghost" asChild>
          <Link href={`/shared-cases/${caseItem.id}`}>
            View Case
          </Link>
        </Button>
      </CardFooter>
