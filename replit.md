...
- **Motion Capture & Virtual Patient System**: Integrates WebRTC for real-time pose detection, AI-powered virtual patient generation, detailed movement analysis, and a Focused Clinical Camera System with GPT-4o Vision AI for detecting clinical signs and automated postural analysis. Supports WebSocket-based phone-to-desktop camera linking. A **Foot Lock** layer (`client/src/utils/footLock.ts`, default ON) detects per-foot support phase from MediaPipe ankle/heel/foot-index velocity, visibility, and floor-proximity (with hysteresis), then re-anchors the avatar root to the planted foot's first-planted screen position so feet stay pinned during torso wobble. The same tracker low-pass-filters the torsoUp/Forward/Right basis (α=0.18) before hip/knee dot products are taken, and `Posesmoother` multiplies planted-leg joint noise by 2.5× for adaptive damping. Wired through both `CameraPoseCapture` and `FocusedCameraCapture` (local + phone full_body paths) with a "Foot lock" toggle and per-foot planted/swinging Badges.
