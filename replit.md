...
- **Biomechanical Systems**: Includes a bidirectional muscle-joint system, 3D force visualization, and a biomechanical clinical assessment system for patient digital twins and injury risk scoring. The bidirectional engine is fully wired on both PhysioGPT and TestSkeletonNew pages — muscle slider changes (activation, tension, inhibition, length) automatically adjust skeleton joint angles, and skeleton slider changes automatically update muscle states, fascial chain tensions, and kinetic chain integrity scores. Controlled via "Muscles Move Bones" toggle (ON by default). Includes full cross-muscle feedback: reciprocal inhibition (agonist activation → antagonist inhibition), fascial chain propagation (tension spreads along myofascial chains to connected muscles), and chain-driven skeletal effects (propagated tension creates secondary joint adjustments at 15% strength factor). Data flow: `modelConfig` → `muscleDrivenEffects` → `effectiveModelConfig` → `baseMuscleTensions` → `chainPropagation` → `chainDrivenJointEffects` → `finalModelConfig` (viewer). Cross-muscle effects feed into `muscleAnalysis` via `CrossMuscleEffects` interface. Muscle panel shows RI (yellow) and FC (cyan) badges on affected muscles. Muscle ID aggregation layer (`aggregateOverridesToGroups` in bidirectionalMuscleJoint.ts) maps granular muscle IDs (e.g., `l_glut_max`) to group IDs (e.g., `glute_l`) used by `MUSCLE_JOINT_ACTIONS`. **Influence Ripple System** (`muscleInfluenceMap.ts`): When a muscle is modified, computes and displays the full domino effect across three pathways — reciprocal inhibition (yellow), fascial chain (cyan), and kinetic chain (orange). Shows colored left borders on influenced muscles, "influenced" count badges on group headers, and an expandable "Influence Ripple" panel under selected modified muscles listing all downstream effects with pathway type, delta values, chain names, and resulting clinical status changes.
...
- **Phone-to-Desktop Camera Link**: WebSocket-based phone camera relay allowing practitioners to use their phone as a remote camera for the Focused Clinical Camera System. Generates QR code/link for instant phone connection, streams live video from phone to desktop via WebSocket relay rooms, supports region selection sync between devices, and integrates directly with the existing AI analysis pipeline for clinical sign detection.
...
- **Treatment Priority Engine** (`treatmentPriorityEngine.ts`): Auto-generates ranked treatment targets from muscle analysis, influence map, chain integrity, and pain markers. Each target includes: clinical status, treatment action (release/stretch/strengthen/activate/mobilize/stabilize), specific techniques with dosage, root cause vs. compensation classification, pain correlations (direct/referred/compensatory), and chain context. Treatment Priority Panel in the muscle sidebar shows the full plan with collapsible target cards, treatment sequence, syndrome detection, and a "Generate AI Treatment Plan" button that sends all biomechanical data to GPT-4o for a comprehensive clinical treatment synthesis. **Evidence-Based Upgrades**: Each technique now includes `evidenceGrade` (A/B/C/Expert), real published `references` (with PubMed IDs), and `guidelineSource` (ACSM/JOSPT). Techniques sorted by evidence quality (A first). UI shows colored evidence grade badges (green A, yellow B, gray C/Expert), expandable references with PubMed links, and guideline source pills. **SYNDROME_PROTOCOLS** database maps detected syndromes (upper_cross, lower_cross, layer_syndrome) to structured Janda/Sahrmann protocols with sequenced techniques, phase recommendations, and clinical references — displayed as expandable protocol cards in the Treatment Panel. **Contraindication System** (`checkContraindications`): 5 clinical safety rules cross-check each target against the full picture (spasm+adjacent inhibition → neural screening, bilateral involvement → systemic cause, severe pain+spasm → avoid aggressive stretch, multiple critical chains → stabilize first, syndrome+acute pain → address acute first). Contraindications displayed as colored alerts (amber caution, orange warning, red stop) with clinical reasoning.
- **Unified Pain Investigation System**: Automatically connects ALL analysis engines (forces, muscles, fascial chains, scars, kinetic chains) when a pain marker is placed. Features a Pain Driver Ranking Engine (`painDriverEngine.ts`) that synthesizes 7 driver categories (structural, biomechanical, myofascial, fascial_chain, scar_tissue, referred, compensatory) with evidence scores 0-100. Cross-system correlation engine (`crossSystemCorrelation.ts`) extended with fascial chain tension and scar data. Postural force engine accepts fascial tension modifiers (5-25% increase range). AI clinical reasoning prompt enriched with fascial chain data, scar tissue analysis, and ranked pain drivers. Pain Driver section in Clinical Reasoning Panel shows ranked contributors with evidence bars, severity badges, mechanism descriptions, and synthesized narrative.
