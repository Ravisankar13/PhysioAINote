...
- **Treatment Decision Engine**: A structured 8-module pipeline (`server/services/treatmentDecisionEngine.ts`) that bridges clinical reasoning to actionable treatment direction. Modules: Candidate Builder → Matching → Risk Filter → Ranking → Priority Classifier → Target Mapper → Review Logic → Explainability. Consumes `ClinicalReasoningResult` from the Structured analysis tab and produces three-tier intervention lists (Primary / Adjunct / Avoid-Defer) with symptom-relief vs root-cause badges, evidence grades, dosage, rationale, risk flags, and transparent explainability. Includes a Review Schedule based on irritability and stage. API: `POST /api/treatment-decision/analyze`. UI: "Decision" tab (emerald theme) in `ClinicalReasoningPanel` via `DecisionTab.tsx`. Auto-refreshes when structured reasoning data updates.
...
- **Injury Mechanism Engine ("Why It Happened")**: Answers the clinical question "why did this injury happen?" by synthesizing data from the cross-system correlation engine, pathology compensation engine, postural force engine, and kinetic chain explorer. Produces causal chain flowcharts (root cause → intermediate mechanism → symptom), load redistribution before/after bars, a 2D COM shift diagram, compensation pattern cards, and matched kinetic chain dysfunctions. Engine: `client/src/lib/injuryMechanismEngine.ts`. Panel: `client/src/components/skeleton/InjuryMechanismPanel.tsx`. Activated via "Mechanism" toolbar button in the 3D viewer; renders as a dark overlay panel matching the Risk & Prognosis dashboard styling. All data is live-reactive to posture sliders, pain markers, pathology overrides, and force analysis changes.
- **What-If Clinical Simulation**: A "What-If" tab inside the Injury Mechanism panel that lets practitioners simulate clinical interventions ("What if glute strength improves 20%?") and instantly see predicted changes in risk scores, joint forces, and compensation patterns. Features: 8 preset scenario buttons for common interventions (glute strengthen, core stability, ankle DF, hip flexor stretch, T-spine mob, scapular strengthen, pec stretch, deep neck flexors), a custom scenario builder with muscle/joint picker + intervention type (strengthen/stretch/mobilize/offload) + magnitude slider, stackable scenarios for combined interventions, before/after comparison dashboard with delta bars, force change percentages, risk level transitions, compensation resolution tracking, and "Apply to Skeleton" / "Reset" controls. Engine: `client/src/lib/whatIfSimulationEngine.ts`. Panel: `client/src/components/skeleton/WhatIfSimulationPanel.tsx`. Purely client-side — no backend needed.
