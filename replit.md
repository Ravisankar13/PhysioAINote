...
- **Advanced Clinical Analysis**:
    ...
    - **Goal-Driven Recovery Engine**: Defines measurable recovery targets per condition and drives treatments to close the gap. Recovery goals are exclusively AI-generated via `POST /api/recovery-goals/generate` (GPT-4o) from the full clinical context. Loading spinner shows while generating; error message shown on failure. Goals auto-regenerate when patient factors or clinical state change (500ms debounce, AbortController, cache).
