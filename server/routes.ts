...
const upload = multer({
  ...
  limits: {
    fileSize: 100 * 1024 * 1024, // 100 MB file size limit for longer recordings
  },
  ...
  app.post("/api/evidence-engine/query", ensureAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const normalizedInput: import("./services/evidenceEngine").EvidenceQueryInput = {
        ...
        sling: d.sling ? {
          weakLinks: Array.isArray(d.sling.weakLinks) ? d.sling.weakLinks.filter((w: unknown) => typeof w === 'string') : undefined,
          systemFailures: Array.isArray(d.sling.systemFailures) ? d.sling.systemFailures.filter((s: unknown) => typeof s === 'string') : undefined,
        ...
        patientContext: d.patientContext ? {
          goals: Array.isArray(d.patientContext.goals) ? d.patientContext.goals.filter((g: unknown) => typeof g === 'string') : undefined,
          sport: typeof d.patientContext.sport === 'string' ? d.patientContext.sport : undefined,
          equipment: Array.isArray(d.patientContext.equipment) ? d.patientContext.equipment.filter((e: unknown) => typeof e === 'string') : undefined,
    ...
    try {
      ...
      const treatmentPlanInputSchema = z.object({
        decisionResult: z.object({
          ...
          stage: z.string().optional(),
          irritability: z.string().optional(),
          topEvidenceOptions: z.array(z.object({
            id: z.string(),
            name: z.string(),
            dosage: z.string(),
            rationale: z.string(),
            ...
            expertApproach: z.string().optional(),
  ...
  app.post("/api/recovery-goals/generate", ensureAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const recoveryGoalInputSchema = z.object({
        ...
        muscleStates: z.array(z.object({
          muscleId: z.string(),
          tension: z.number(),
        })).optional().default([]),
        compensationPatterns: z.array(z.string()).optional().default([]),
        posturalDeviations: z.array(z.string()).optional().default([]),
        slingAnalysis: z.array(z.object({
          slingName: z.string(),
          integrity: z.number(),
        })).optional().default([]),
        patientFactors: z.object({
          ...
          steroidInjectionHistory: z.boolean().optional(),
          steroidInjectionCount: z.number().optional(),
          previousEpisodes: z.number().optional(),
          chronicity: z.string().optional(),
          irritability: z.string().optional(),
          compliance: z.number().optional(),
          psychologicalRisk: z.string().optional(),
          sleepQuality: z.string().optional(),
          activityLevel: z.string().optional(),
          sideAffected: z.string().optional(),
          medicationUse: z.array(z.string()).optional(),
          comorbiditiesNotes: z.string().optional(),
        }).optional(),
        extractionSummary: z.string().optional().default(""),
      });

      const parsed = recoveryGoalInputSchema.safeParse(req.body);
      if (!parsed.success) {
        return res.status(400).json({ error: "Invalid input", details: parsed.error.format() });
      }
      ...
      const patientSummary = pf
        ? [
            pf.age !== null && pf.age !== undefined ? `Age: ${pf.age}` : null,
            pf.bmi && pf.bmi !== 'normal' ? `BMI: ${pf.bmi}` : null,
            pf.diabetes && pf.diabetes !== 'none' ? `Diabetes: ${pf.diabetes}` : null,
            pf.smoking && pf.smoking !== 'never' ? `Smoking: ${pf.smoking}` : null,
...
Return JSON: {"conditionId":"ai_generated_<snake_case>","conditionName":string,"romTargets":[{"jointId":string,"label":string,"targetDegrees":number,"normalDegrees":number,"recoveryPercent":number}],"painTarget":number,"muscleTensionTargets":[{"muscleId":string,"targetTensionMin":number,"targetTensionMax":number}],"slingTargets":[{"slingName":string,"targetIntegrity":number}],"postureTargets":[{"deviationType":string,"boneName":string,"targetAngle":number,"axis":"x"|"y"|"z"}],"compensationResolutionTarget":number,"functionalGoals":[{"label":string,"metric":string,"targetValue":number,"unit":string}],"riskScoreTarget":number,"strengthTarget":number,"jointStressTarget":number,"overallRomRecoveryPercent":number}`;
      ...
      const goalResponseSchema = z.object({
        ...
        romTargets: z.array(z.object({
          jointId: z.string(),
          label: z.string(),
          targetDegrees: z.number(),
          normalDegrees: z.number(),
          recoveryPercent: z.number(),
        })).default([]),
        painTarget: z.number().min(0).max(100).default(10),
        muscleTensionTargets: z.array(z.object({
          muscleId: z.string(),
          targetTensionMin: z.number(),
          targetTensionMax: z.number(),
        })).default([]),
        slingTargets: z.array(z.object({
          slingName: z.string(),
          targetIntegrity: z.number(),
        })).default([]),
        postureTargets: z.array(z.object({
          deviationType: z.string(),
          boneName: z.string(),
          targetAngle: z.number(),
          axis: z.enum(["x", "y", "z"]),
        })).default([]),
        compensationResolutionTarget: z.number().default(90),
        functionalGoals: z.array(z.object({
          label: z.string(),
          metric: z.string(),
          targetValue: z.number(),
          unit: z.string(),
        })).default([]),
        riskScoreTarget: z.number().default(15),
        strengthTarget: z.number().default(75),
        jointStressTarget: z.number().default(20),
        overallRomRecoveryPercent: z.number().default(90),
      });

      const raw = JSON.parse(content);
      const validated = goalResponseSchema.safeParse(raw);
      if (!validated.success) {
        console.error("Recovery goal AI response validation failed:", validated.error.format());
        return res.status(502).json({
          error: "AI response did not match expected format",
          raw,
        });
      ...
      console.error("Recovery goals generation error:", error);
      const message = error instanceof Error ? error.message : "Unknown error";
      res.status(500).json({ error: "Failed to generate recovery goals", details: message });
    }
  });
...
- Return empty arrays if no relevant information is found for that category`;
  ...
  app.post('/api/clinical-text/parse', ensureAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const qaContext = Array.isArray(context) ? context as Array<{question: string; answer: string}> : [];
      ...
      const response = await replitOpenai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `You are an expert clinical physiotherapy AI and diagnostic prediction engine. Your job is to take a patient description — which may be a diagnosis name, symptoms, body type clues, or any combination — and PREDICT the full skeletal presentation for 3D visualization.

CRITICAL INSTRUCTION: You are a PREDICTION engine, not just an extractor. When given a diagnosis (e.g., "Baastrup's disease"), you must use your clinical knowledge to predict ALL the postural, muscular, and pain findings that would typically present with that condition. When given body type clues (e.g., "big bum", "tall and thin", "elderly", "obese"), factor those into the predicted presentation.

For each finding, mark its "confidence" as:
- "confirmed" = explicitly stated by the patient/user
- "predicted" = you inferred this based on clinical knowledge of the condition

Return a JSON object with:

1. "pain_markers": Array of pain/symptom locations (both confirmed AND predicted). Each has:
   - "anatomical_label": exact match from: C1-C2, C3-C4, C5-C6, C7-T1, T1-T4, T5-T8, T9-T10, T11-T12, L1-L2, L3, L4-L5, L5-S1, Sacrum, Left SI Joint, Right SI Joint, Left Greater Trochanter, Right Greater Trochanter, Left IT Band, Right IT Band, Left Calf, Right Calf, Left Achilles Tendon, Right Achilles Tendon, Left Forearm, Right Forearm, Left Biceps, Right Biceps, Left ASIS, Right ASIS, Left PSIS, Right PSIS, Left Patella, Right Patella, Left Medial Knee Joint Line, Right Medial Knee Joint Line, Left Lateral Knee Joint Line, Right Lateral Knee Joint Line, Left Popliteal Fossa, Right Popliteal Fossa, Left Medial Malleolus, Right Medial Malleolus, Left Lateral Malleolus, Right Lateral Malleolus, Left Calcaneus, Right Calcaneus, Left Plantar Fascia, Right Plantar Fascia, Left Acromioclavicular Joint, Right Acromioclavicular Joint, Left Glenohumeral Joint, Right Glenohumeral Joint, Left Subacromial Space, Right Subacromial Space, Left Lateral Epicondyle, Right Lateral Epicondyle, Left Medial Epicondyle, Right Medial Epicondyle, Left Radial Head, Right Radial Head, TMJ Left, TMJ Right, Occiput, Sternum, Left Carpal Tunnel, Right Carpal Tunnel
   ...
   - "description": brief clinical description explaining why this finding is expected
   - "confidence": "confirmed" or "predicted"

2. "muscle_states": Array of affected muscles (both confirmed AND predicted). Each has:
...
3. "postural_deviations": Object mapping posture parameters to degree values. Predict ALL postural changes associated with the condition. Keys are dot-notation paths:
   - "spine.thoracicKyphosis", "spine.lumbarLordosis", "spine.forwardHead", "spine.scoliosis", "spine.lateralShift", "spine.flexion"
   ...
   Values should be realistic clinical degrees (e.g. 20 for moderate kyphosis, 15 for anterior pelvic tilt).
...
5. "clinical_summary": A 1-2 sentence summary of the predicted clinical picture, explaining the diagnosis and its typical presentation.
...
8. "follow_up_questions": Array of questions to ask the clinician to refine the prediction. Each has:
   ...
   - "clinical_relevance": brief explanation of why this question matters for the prediction

Generate 2-5 follow-up questions when important clinical details are missing. Questions should help determine: severity, chronicity, laterality, aggravating/easing factors, or associated conditions that would change the skeletal presentation. Do NOT ask questions if the description is already comprehensive.

EXAMPLES of good predictions:
- "Baastrup's disease" → predict increased lumbar lordosis (spine.lumbarLordosis: 25), pain at L4-L5 interspinous area, erector spinae hypertonicity, potential hip flexor tightness, ask about chronicity and extension vs flexion preference
- "Frozen shoulder" → predict restricted GH ROM, compensatory scapular elevation, potential upper trap spasm, ask about which shoulder and stage (freezing/frozen/thawing)
- "Elderly with kyphosis" → predict increased thoracic kyphosis, forward head posture, potential cervical lordosis loss, weakened lower traps and deep neck flexors`
          },
          {
            role: "user",
...
Be specific to THIS skeleton's findings. Every root cause must reference a specific finding from the assessment data. Treatment interventions must directly target identified root causes.`;
  ...
  app.get('/api/documents/download/:documentId', async (req, res) => {
    try {
      ...
      let document = await storage.getGeneratedDocument(documentId);
      
      if (!document) {
        // If not found and sessionId provided, try session-based lookup
        if (sessionId) {
          console.log(`Document not found by ID, trying session ${sessionId}`);
