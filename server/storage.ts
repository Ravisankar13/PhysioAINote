  ...
  async getSoapNotes(userId: number): Promise<SoapNote[]> {
    try {
      const result = await db
        .select()
        .from(soapNotes)
        .where(eq(soapNotes.user_id, userId))
        .orderBy(desc(soapNotes.created_at));
  ...
  async getUserSoapVirtualPatients(userId: number): Promise<SoapVirtualPatient[]> {
    try {
      const result = await db
        .select()
        .from(soapVirtualPatients)
        .where(eq(soapVirtualPatients.user_id, userId))
        .orderBy(desc(soapVirtualPatients.created_at));
